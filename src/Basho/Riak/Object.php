...
class Object
{
     ...
     *                       is generated by server when store(...)
     *                       is called.
     */
    public function __construct(Client $client, $bucket, $key = null)
    {
        $this->meta = array();
        $this->indexes = array();
     ...
     *
     * @param string         $indexName     Name
     * @param string         $indexType     Must be one of 'int' or 'bin' - the
     *                                      only two index types supported
     *                                      by Riak
     * @param string|integer $explicitValue Optional. If provided, uses this
     *                                      value explicitly.  If not provided,
     *                                      this will search the object's data
     *                                      for a field with the name
     *                                      $indexName, and use that value.
     *
     * @return Object
     */
     ...
     * @param array|string|integer $values
     *
     * @return Object
     */
    ...
    public function removeIndex($indexName, $indexType = null, $explicitValue = null)
    {
        ...
        if (false
                !== ($position = array_search($explicitValue,
                        $this->indexes[$index]))) {
     ...
     * @param string $indexType Index type. Must be one of 'int' or 'bin'
     *
     * @return boolean
     */
     ...
     *
     * @throws \Exception if trying to use auto indexes for non-array
     * @return Object
     */
    public function store($w = null, $dw = null)
    {
        ...
        if (!empty($this->autoIndexes) && !is_array($this->data)) {
            throw new \Exception(
                    'Unsupported data type for auto indexing feature.  Object must be an array to use auto indexes.');
        }
        ...
        count($this->autoIndexes) > 0 ? $this->meta['x-rc-autoindex'] = json_encode(
        $this->autoIndexes)
        : $this->meta['x-rc-autoindex'] = null;
        count($collisions) > 0 ? $this->meta['x-rc-autoindexcollision'] = json_encode(
        $collisions)
        ...
        foreach ($this->indexes as $index => $values) {
            ...
            . join(', ', array_map('urlencode', $values));
        }
    ...
    public function populate($response, array $expectedStatuses)
    {
            ...
            if (preg_match('~^x-riak-([^-]+)-(.+)$~', $key, $matches)) {
                switch ($matches[1]) {
                    case 'index':
                        ...
                        $this
                        ->setIndex($index, $type,
                                array_map('urldecode', explode(', ', $val)));
                        break;
                    case 'meta':
        ...
        if (isset($this->meta['x-rc-autoindex'])) {
            ...
            $this->autoIndexes = json_decode($this->meta['x-rc-autoindex'],
            true);
            $collisions = isset($this->meta['x-rc-autoindexcollision']) ? json_decode(
                    $this->meta['x-rc-autoindexcollision'], true)
                    : array();
    ...
    private function populateLinks($linkHeaders)
    {
        ...
        foreach ($linkHeaders as $linkHeader) {
            ...
            $result = preg_match(
                    ...
                    $linkHeader, $matches);
            if ($result == 1) {
                $this->links[] = new Link(urldecode($matches[2]),
                        urldecode($matches[3]), urldecode($matches[4]));
            }
        }
